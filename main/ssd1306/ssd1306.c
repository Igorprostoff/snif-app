...
{
	ESP_LOGD(log_tag, "software_scroll start=%d end=%d _pages=%d", start, end, dev->_pages);
	if (start < 0 || end < 0) {
		dev->_scEnable = false;
...
{
	ESP_LOGD(log_tag, "dev->_scEnable=%d", dev->_scEnable);
	...
	while(1) {
		...
		ESP_LOGD(log_tag, "srcIndex=%d dstIndex=%d", srcIndex,dstIndex);
		...
		ESP_LOGD(log_tag, "_valid=%d", dev->_page[dstIndex]._valid);
