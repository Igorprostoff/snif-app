...
void i2c_init(SSD1306_t * dev, int width, int height) {
	...
	if (espRc == ESP_OK) {
		ESP_LOGI(log_tag, "OLED configured successfully");
	} else {
		ESP_LOGE(log_tag, "OLED configuration failed. code: 0x%.2X", espRc);
	}
...
void i2c_hardware_scroll(SSD1306_t * dev, ssd1306_scroll_type_t scroll) {
	...
	if (espRc == ESP_OK) {
		ESP_LOGD(log_tag, "Scroll command succeeded");
	} else {
		ESP_LOGE(log_tag, "Scroll command failed. code: 0x%.2X", espRc);
	}
